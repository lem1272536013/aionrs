...
async fn test_spawn_single_agent() {
    ...
    let spawner = AgentSpawner::new(provider, test_config(), std::env::temp_dir(), vec![]);
...
async fn test_spawn_shares_provider() {
    ...
    let spawner = AgentSpawner::new(Arc::clone(&provider_dyn), test_config(), std::env::temp_dir(), vec![]);
